 ...
 * @LastEditTime: 2021-03-12 17:02:35
 */
...
OPERATE_RET app_radar_init(IN APP_RADAR_MODE mode)
{
...
VOID app_report_all_dp_status(VOID)
{
    ...
    dp_arr[0].dpid = DPID_PRESENCE_STATE;
    dp_arr[0].type = PROP_ENUM;
    dp_arr[0].time_stamp = 0;
    dp_arr[0].value.dp_value = app_ctrl_data.presence_state;

    dp_arr[1].dpid = DPID_TARGET_DISTANCE;
    dp_arr[1].type = PROP_VALUE;
    dp_arr[1].time_stamp = 0;
    dp_arr[1].value.dp_value = app_ctrl_data.target_distance;
